...
try {
  ...
  await page.waitForSelector('.ct-drawer-list')
  console.log('interaction shell: regions available', await page.locator('.ct-drawer-list > button').allTextContents())
  await page.locator('.ct-drawer-list > button').nth(2).click()
  await page.waitForSelector('.ct-transit')
  if (!/灯塔区|Lighthouse Ward/.test(await page.locator('.ct-chat-head__identity').innerText())) throw new Error('region changed before transit confirmation')
  await page.screenshot({ path: '_qa/ui/continuity-shared-world-transit-platform-layout-390x844.png', fullPage: true })
  await page.locator('.ct-transit section > button').first().click()
  await page.waitForSelector('.ct-transit', { state: 'detached' })
  await page.waitForFunction(() => /雨棚市场|Rain Market/.test(document.querySelector('.ct-chat-head__identity')?.textContent || ''))
  console.log('interaction shell: transit confirmed and market selected')
  const initialVersion = await page.locator('.ct-action-feedback').count()
  await page.locator('.ct-quick-replies button').nth(0).click()
  ...
  await page.locator('.ct-quick-replies button').nth(1).click()
  ...
  const send = page.locator('.ct-message-composer form button[type="submit"]')
  if (!(await send.isDisabled())) throw new Error('empty message was submittable')
  await page.locator('.ct-message-composer textarea').fill('把灯留给下一位经过这里的人。')
  ...
  console.log('interaction shell ok · cover gate · scrollTop 0 · transit gate · one-click action · message-only composer')
} finally {
  await browser.close()
}
