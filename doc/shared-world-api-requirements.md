...
- staging 可通过显式 `?api_base=` 指向独立实验 Worker；production beta 部署在当前游戏 UUID 路径。正式前端必须用 `API_BASE = "/" + GAME_ID` 请求同 Worker，禁止 `.env.production`、`VITE_*_API_BASE` 或绝对 URL 写死源游戏 UUID；Remix 替换 GAME_ID 后自动进入新 Worker 与新数据库。世界真源均为 DO SQLite，前端通过 Remote Gateway 接入。
