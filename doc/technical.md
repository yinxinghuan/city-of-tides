...
- 封面和开场图使用独立宽图资源。运行时场景导演 v4 保留与本轮正文一致的 AI 镜头提议，再叠加当前可见剧情、最新地点和统一画风；若提议命中返航渡轮等开场残留词，则改用本地正向场景描述。残留词只参与本地检测，不会以“禁止出现”清单再次发送给生图模型。
- 每个场景图同时保存 `player / environment / others` 主体分类。只要玩家主角在画面任何位置出现，尤其执行本轮明确动作时，就必须使用头像 `ref_url`；只有无人空镜或只出现其他角色时不传。旧存档中未完成的 v1–v3 图片任务在恢复时升级为 v4；已完成的历史图片不自动重画。
