...
- `createRecoveryChoices()` 只在危险态复述同一个具体威胁并选取 cartridge 的单步应对手段；平静态零有效选项时保持空 tray，由自由输入继续，不生成可能形成固定点的通用转场。
...
普通故事选择只有一次轻确认，正文、常规数值和图片完成静音；持久结果才追加语义提示。Cartridge 合成反馈上限为 `0.045`，故事引擎再乘 `0.52`。共享世界 `district/trace/reply/anchor/warning` 提示增益不高于 `0.011`，并以 `360 ms` 全局冷却合并连续事件。

## 2026-08-23 阅读优先 A/B 配乐

共享世界常规阅读使用低密度潮城底乐 A；只有永久锚点完成这一类权威关键事件使用 B。B 暂停 A、结束后恢复 A，并有 180 秒客户端冷却；静音、后台、重连和历史回放均不会补播 B，环境声保持独立。
