...
function sendAWMessage(message: string): void {
  const host = window as any
  try {
    if (host.webkit?.messageHandlers?.aigram) host.webkit.messageHandlers.aigram.postMessage(message)
    else if (api_origin) window.parent.postMessage(message, new URL(api_origin).origin)
  } catch { /* ignore */ }
}

export function openAigramProfile(userId: string): void {
  if (!userId) return
  sendAWMessage(`AW.PROFILE.OPEN-${btoa(JSON.stringify({ id: userId }))}`)
}
